...
const worker = createWorker<MatchmakingJobData>('matchmaking-jobs', async (job: Job<MatchmakingJobData>) => {
  const { userId, eloRating, duration, mode, timePerQuestion } = job.data;
  console.log(`[Worker][Matchmaking] Processing job #${job.id} for user ${userId} (Mode: ${mode}, Attempt #${job.attemptsMade + 1})`);
  try {
    // Call the service to process the new player for matchmaking
    await matchmakingService.processNewPlayer(userId, eloRating, duration, mode, timePerQuestion);
    console.log(`[Worker][Matchmaking] Successfully processed job #${job.id} for user ${userId}.`);
  } catch (error) {
    console.error(`[Worker][Matchmaking] Failed to process job #${job.id} for user ${userId}:`, error);
