...
// --- HELPER COMPONENT: NEW RESULTS GRAPH ---
const ResultsGraph = ({ results, players, myUserId }: { results: Results, players: Player[], myUserId: string }) => {
  ...
  const processData = (playerResults: AnswerData[]) => {
    return playerResults.map(res => {
      if (res.action === 'timeout') return 1; // Low value for timeout
      if (res.action === 'answered' && !res.correct) return 2; // Mid value for incorrect
      if (res.action === 'answered' && res.correct) return res.timeTaken / 1000; // Time in seconds for correct
      return 0;
    });
  };
  ...
  const data = {
    ...
    datasets: [
      {
        label: `${me?.username || 'You'} (Time/Status)`,
      ...
      {
        label: `${opponent?.username || 'Opponent'} (Time/Status)`,
  ...
  const options = {
    ...
    plugins: {
      ...
      title: { display: true, text: 'Performance Analysis (Lower is Better)' },
      tooltip: {
          callbacks: {
              label: function(context) {
                  ...
                  if (value === 1) return label + 'Timeout';
                  if (value === 2) return label + 'Incorrect';
                  return label + `${value.toFixed(2)} seconds`;
              }
          }
      }
    },
    scales: { y: { beginAtZero: true, title: { display: true, text: 'Time (s) / Status (1=Timeout, 2=Incorrect)' } } }
  };
...
export const FastestFingerGame = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('ff:player_answered', (data) => {
      ...
      if (isMe && !data.correct) setWaitingMessage('Incorrect answer! Waiting for opponent...');
    });
  ...
  const getOptionStyle = (optionId: string) => {
    if (correctOptionId) { // Once the correct answer is revealed
    ...
    if (selectedOptionId === optionId) return styles.optionSelected; // While waiting for result
    return styles.option;
  };
        ...
        <div style={styles.card}>
          ...
          <div style={styles.optionsGrid}>
            {currentQuestion.options.map(option => (
              <button key={option.id} onClick={() => handleAnswerClick(option.id)} disabled={hasAnswered} style={getOptionStyle(option.id)}>
                {option.text}
              </button>
            ))}
          </div>
        ...
        <div style={styles.card}>
          ...
          {gameResults && (
            <div style={styles.graphContainer}>
              <ResultsGraph results={gameResults} players={players} myUserId={userId} />
            </div>
          )}
