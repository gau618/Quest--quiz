...
const ResultsGraph = ({ results, players, myUserId }: { results: Results, players: Player[], myUserId: string | null }) => {
...
export const FastestFingerGame = () => {
  ...
  const myParticipantId = useMemo(() => players.find(p => p.userId === authInfo?.userId)?.participantId, [players, authInfo]);
  const opponent = useMemo(() => players.find(p => p.userId !== authInfo?.userId), [players, authInfo]);
  ...
  useEffect(() => {
    ...
    const handleMatchFound = (data: { sessionId: string; players: Player[], duration: number, timePerQuestion: number, totalQuestions: number }) => {
    ...
    const handleNewQuestion = (data: { question: Question; questionNumber: number; timeLimit: number }) => {
    ...
    const handlePointAwarded = (data: { participantId: string; allScores: Record<string, number>; correctOptionId: string }) => {
    ...
    <div style={styles.container}>
      ...
      {gameStatus === 'idle' && (
        <div style={styles.card}>
          ...
          <button onClick={findMatch} style={styles.button} disabled={!authInfo || !isConnected || gameStatus === 'searching'}>
            {gameStatus === 'searching' ? "Finding Match..." : !isConnected ? "Connecting..." : "Find Match"}
          </button>
        </div>
      )}
      ...
      {(gameStatus === 'playing' || gameStatus === 'answered') && currentQuestion && (
        <div style={styles.card}>
          ...
          <div style={styles.scoreBoard}>
            <span>{players.find(p => p.userId === authInfo?.userId)?.username || 'You'}: {myScore}</span>
      ...
      {gameStatus === 'finished' && (
        <div style={styles.card}>
          ...
          {gameResults && authInfo && (<div style={styles.graphContainer}><ResultsGraph results={gameResults} players={players} myUserId={authInfo.userId} /></div>)}
