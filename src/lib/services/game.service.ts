...
function getBotDelay(elo: number, gameMode: GameMode): number {
  const minElo = 600,
    maxElo = 2800;
  let baseMaxDelay, baseMinDelay, minHumanDelay, maxHumanDelay;
  ...
  let baseDelay =
    elo <= minElo
      ...
      ? baseMinDelay
      : baseMaxDelay -
        ((elo - minElo) / (maxElo - minElo)) * (baseMaxDelay - baseMinDelay);
  ...
  if (Math.random() < 0.15 && !(Math.random() < 0.1))
    finalDelay *= 0.4 + Math.random() * 0.4;
  finalDelay *= 0.7 + Math.max(0.5, (elo - minElo) / (maxElo - minElo)) * 0.6;
  return Math.round(
    Math.max(minHumanDelay, Math.min(maxHumanDelay, finalDelay))
  );
}
...
class GameService {
  private getDifficultyFromElo = (elo: number): Difficulty => {
    if (elo < 1300) return Difficulty.HARD;
  ...
  private fetchQuestions = async (
    ...
    const skip =
      total > count ? Math.floor(Math.random() * (total - count)) : 0;
  ...
  private async createSession(
    ...
    if (gameMode === GameMode.FASTEST_FINGER_FIRST) {
      socketService.emitToUsers(
        session.participants.filter((p) => !p.isBot).map((p) => p.userId),
        "ff:match_found",
        {
      ...
      socketService.emitToUsers(
        session.participants.filter((p) => !p.isBot).map((p) => p.userId),
        "match:found",
        { sessionId: session.id, players: playersInfo, duration }
      );
      session.participants.forEach((p) =>
    ...
    console.log(
      `[GameService] Scheduled game end job ${gameEndJobId} in ${duration} minutes.`
    );
  }
  ...
  public async startDuel(p1Id: string, p2Id: string, duration: number) {
    ...
    await this.createSession(
      ...
      0,
      duration,
      this.getDifficultyFromElo(avgElo),
      GameMode.QUICK_DUEL
    );
  }
  public async startBotDuel(pId: string, duration: number) {
    ...
    await this.createSession(
      ...
      1,
      duration,
      this.getDifficultyFromElo(profile.eloRating),
      GameMode.QUICK_DUEL
    );
  }
  ...
  public async sendNextQuestion(sId: string, pId: string) {
    const gsStr = await redis.get(`game_state:${sId}`);
    if (!gsStr) return;
    const gs: GameState = JSON.parse(gsStr);
    if (Date.now() >= gs.endTime) return;
    const idx = gs.userProgress[pId] ?? 0;
    const q = gs.questions[idx];
    if (q) {
      gs.questionSentAt[pId] = Date.now();
      await redis.set(`game_state:${sId}`, JSON.stringify(gs), "KEEPTTL");
      const participant = await prisma.gameParticipant.findUnique({
        where: { id: pId },
      });
      if (participant?.isBot) {
        this._simulateQuickDuelBotAnswer(sId, pId, q);
      } else {
        socketService.emitToParticipant(pId, "question:new", q);
      }
    } else {
      socketService.emitToParticipant(pId, "participant:finished", {
  ...
  private async startFastestFingerQuestion(sId: string) {
    const gsStr = await redis.get(`game_state:${sId}`);
    if (!gsStr) return;
    let gs: GameState = JSON.parse(gsStr);
    if (
      gs.currentQuestionIndex! >= gs.questions.length ||
      Date.now() >= gs.endTime
    ) {
      this.endGame(sId);
      return;
    ...
    gs.questionAnswers = [];
    await redis.set(`game_state:${sId}`, JSON.stringify(gs), "KEEPTTL");
    socketService.emitToRoom(sId, "ff:new_question", {
      question: q,
      questionNumber: gs.currentQuestionIndex! + 1,
      timeLimit: gs.timePerQuestion!,
    });
    ...
    await queueService.dispatch(
      ...
      { delay: gs.timePerQuestion!, jobId }
    );
    await redis.set(`ff_timer_job:${sId}`, jobId);
    console.log(
      `[GameService] Scheduled timeout job ${jobId} for question ${q.id}.`
    );
    ...
    session?.participants.forEach((p) => {
      if (p.isBot)
        this._simulateFastestFingerBotAnswer(sId, p.id, q, gs.timePerQuestion!);
    });
  }
  ...
  public async handleFastestFingerAnswer(
    ...
    if (
      !cQ ||
      cQ.id !== qId ||
      gs.questionAnswers!.some((a) => a.participantId === pId) ||
      Date.now() > gs.questionStartTime! + gs.timePerQuestion!
    )
      return;
    const isCorrect = cQ.correctOptionId === oId;
    gs.questionAnswers!.push({
      participantId: pId,
      optionId: oId,
      timestamp: Date.now(),
      correct: isCorrect,
    ...
    gs.results[pId].push({
      questionId: qId,
      timeTaken: Date.now() - gs.questionStartTime!,
      action: "answered",
      correct: isCorrect,
    });
    await redis.set(`game_state:${sId}`, JSON.stringify(gs), "KEEPTTL");
    socketService.emitToRoom(sId, "ff:player_answered", {
      participantId: pId,
      correct: isCorrect,
    });
    if (isCorrect) {
      ...
      if (
        ...
        correctAnswers[0].participantId === pId
      ) {
        gs.scores[pId] = (gs.scores[pId] || 0) + 1;
        await redis.set(`game_state:${sId}`, JSON.stringify(gs), "KEEPTTL");
        socketService.emitToRoom(sId, "ff:point_awarded", {
          participantId: pId,
          allScores: gs.scores,
        });
        ...
        if (jobIdToCancel) {
          ...
          await redis.del(`ff_timer_job:${sId}`);
          console.log(`[GameService] Cancelled timeout job ${jobIdToCancel}.`);
        }
  ...
  public async processFastestFingerTimeout(
    ...
    const gsStr = await redis.get(`game_state:${sessionId}`);
    if (!gsStr) return;
    let gs: GameState = JSON.parse(gsStr);
    if (gs.questions[gs.currentQuestionIndex!].id !== questionId) {
      console.warn(
        `[GameService] Stale timeout job for Q${questionId}. Ignoring.`
      );
      return;
    }
    ...
    session?.participants.forEach((p) => {
      if (!gs.questionAnswers!.some((a) => a.participantId === p.id))
        gs.results[p.id].push({
          questionId: cQ.id,
          timeTaken: gs.timePerQuestion!,
          action: "timeout",
          correct: false,
        });
    });
  ...
  public async handleAnswer(
    ...
    if (!gsStr) return;
    let gs: GameState = JSON.parse(gsStr);
    if (gs.gameMode === GameMode.FASTEST_FINGER_FIRST)
      return this.handleFastestFingerAnswer(sId, pId, qId, oId);
    ...
    if (gs.questionSentAt[pId]) {
      gs.results[pId].push({
        questionId: qId,
        timeTaken: Date.now() - gs.questionSentAt[pId],
        action: "answered",
        correct: isCorrect,
      });
      delete gs.questionSentAt[pId];
    }
    if (isCorrect) {
      gs.scores[pId] = (gs.scores[pId] || 0) + 10;
      socketService.emitToRoom(sId, "score:update", gs.scores);
    }
    gs.userProgress[pId]++;
    await redis.set(`game_state:${sId}`, JSON.stringify(gs), "KEEPTTL");
  ...
  public async handleSkip(sId: string, pId: string) {
    ...
    const startTime = gs.questionSentAt[pId];
    const question = gs.questions[gs.userProgress[pId] ?? 0];
  ...
  private async _simulateQuickDuelBotAnswer(
    sId: string,
    pId: string,
    q: Question
  ) {
    const delay = getBotDelay(1200, GameMode.QUICK_DUEL);
    const isCorrect = Math.random() < getBotAccuracy(1200);
    const incorrectOpts = q.options.filter((o) => o.id !== q.correctOptionId);
    const chosenId = isCorrect
      ? q.correctOptionId
      : incorrectOpts[Math.floor(Math.random() * incorrectOpts.length)].id;
    setTimeout(() => this.handleAnswer(sId, pId, q.id, chosenId), delay);
  }

  private async _simulateFastestFingerBotAnswer(
    sId: string,
    pId: string,
    q: Question,
    time: number
  ) {
    const delay = getBotDelay(1200, GameMode.FASTEST_FINGER_FIRST);
    const isCorrect = Math.random() < getBotAccuracy(1200);
    const incorrectOpts = q.options.filter((o) => o.id !== q.correctOptionId);
    const chosenId = isCorrect
      ? q.correctOptionId
    ...
    setTimeout(
      () => this.handleFastestFingerAnswer(sId, pId, q.id, chosenId),
      answerDelay
    );
  }
  ...
  public async endGame(sessionId: string) {
    ...
    if (questionJobId) {
      ...
      await redis.del(`ff_timer_job:${sessionId}`);
    }
    const gsStr = await redis.get(`game_state:${sessionId}`);
    if (!gsStr) return;
    ...
    if (gameState.gameMode === GameMode.QUICK_DUEL) {
      for (const pId in gameState.questionSentAt) {
        ...
        if (startTime) {
          ...
          if (q)
            gameState.results[pId].push({
              questionId: q.id,
              timeTaken: gameState.endTime - startTime,
              action: "timeout",
              correct: false,
            });
        }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const p of session.participants) {
        await tx.gameParticipant.update({
          where: { id: p.id },
          data: { score: gameState.scores[p.id] || 0 },
        });
      }
      ...
      if (humanPlayers.length === 2) {
        ...
        const scoreA =
          gameState.scores[p1.id] > gameState.scores[p2.id]
            ? 1
            : gameState.scores[p1.id] === gameState.scores[p2.id]
            ? 0.5
            : 0;
        ...
        await tx.userProfile.update({
          ...
          data: { eloRating: newP1Elo },
        });
        await tx.userProfile.update({
          where: { userId: p2.userId },
          data: { eloRating: newP2Elo },
        });
      }
    });

    const eventName =
      gameState.gameMode === GameMode.FASTEST_FINGER_FIRST
        ? "ff:game_end"
        : "game:end";
