...
const worker = createWorker<GameTimerJobData>(
  ...
  async (job: Job<GameTimerJobData>) => {
    ...
    console.log(
      `[Worker][GameTimer] Processing question timeout for session ${sessionId}, question ${questionId}`
    );
    try {
      await gameService.processFastestFingerTimeout(sessionId, questionId);
      console.log(
        `[Worker][GameTimer] Finished processing timeout for session ${sessionId}`
      );
    } catch (error) {
      console.error(
        `[Worker][GameTimer] Error processing game timer job for session ${sessionId}:`,
        error
      );
      throw error;
    }
