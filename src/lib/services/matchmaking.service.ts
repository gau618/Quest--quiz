...
class MatchmakingService {
  ...
  public async processNewPlayer(userId: string, elo: number, duration: number, mode: GameMode, timePerQuestion?: number) {
    ...
    const playerDataJson = JSON.stringify({ timestamp: Date.now(), timePerQuestion });

    const result = await redis.eval(
      matchmakingScript, 1, queue,
      userId, elo.toString(), minElo.toString(), maxElo.toString(), playerDataJson
    ) as string | null;
    ...
    if (result) {
      ...
      if (mode === GameMode.FASTEST_FINGER_FIRST) {
        try {
          ...
          console.error(`[MatchmakingService] Error parsing opponent data: '${opponentDataStr}'`, e);
        }
      }

      if (mode === GameMode.FASTEST_FINGER_FIRST) {
        const finalTimePerQuestion = Math.min(timePerQuestion || 30000, opponentTimePerQuestion || 30000);
        await gameService.startFastestFinger(userId, opponentId, duration, finalTimePerQuestion);
      } else {
        await gameService.startDuel(userId, opponentId, duration);
      }
    } else {
      console.log(`[MatchmakingService] Player ${userId} added to queue '${queue}'.`);
    }
  }
        ...
        for (const playerString of allPlayers) {
          ...
          if (Date.now() - playerData.timestamp > TIMEOUT_SECONDS * 1000) {
            if (await redis.zrem(queue, playerString) > 0) {
              console.log(`[MatchmakingService] Player ${userId} timed out. Matching with bot for ${mode}.`);
              if (mode === GameMode.FASTEST_FINGER_FIRST) {
                await gameService.startFastestFingerBot(userId, duration, playerData.timePerQuestion || 30000);
              } else {
